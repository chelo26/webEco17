...
def trainCTRmodel(training_events, training_labels):
    ...
    for key in training_events.keys():
        ...
        #lr = SGDClassifier(class_weight={1: pos_weight, 0: neg_weight},penalty="l1",loss="log")
...
def RTB_simulation(model, validation_path, training_path,
                   start_budget = 25000000, lambda_const=5.2e-7, c=20, data_type="Validation"):  # param is the dictionary with the bidprice per advertiser
    impressions = 0
    clicks = 0
    #number_rows=0
    budget=start_budget
...
def RTB_simulation_test(model, validation_path, start_budget = 25000000,
                        lambda_const=5.2e-7, c=20, data_type="test"):  # param is the dictionary with the bidprice per advertiser
    ...
    with open(validation_path, 'r') as csvfile:
        ...
        for row in reader:
            ...
            current_bid=np.power((pCTR+np.square((c**2*lambda_const**2+pCTR**2))/(c*lambda_const)),(1/3))- \
                                np.power(((c*lambda_const)/(pCTR+np.square((c**2*lambda_const**2+pCTR**2)))),(1/3))
            results.append((row[2],advertiser))
...
if __name__=="__main__":
    ...
    val_best_CTR,results=RTB_simulation(models_best_CTR, validation_path, training_path ,data_type="Validation")
    ...
    results_test = RTB_simulation_test(models_best_CTR, test_path, c=33, data_type="Test")

    #optimal_c=find_optimal_c(models_best_CTR,validation_path,training_path)
    # optimal_lambda = find_optimal_lambda(models_best_CTR, validation_path, training_path)
    #optimal_params = find_optimal_params(models_best_CTR, validation_path, training_path)

    data=results_to_csv(results_test)